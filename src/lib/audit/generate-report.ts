...
const finalReport = z.object({ overallScore: z.number().min(0).max(100), executiveSummary: z.string(), priorities: z.array(finding).min(3).max(5), sections: z.array(expert).length(8), copySuggestions: z.array(z.object({ label: z.enum(["headline", "subheadline", "cta", "hero"]), before: z.string(), after: z.string(), alternative: z.string(), rationale: z.string() })).min(3).max(4), quickWins: z.array(z.string()).min(3).max(10), longTermImprovements: z.array(z.string()).min(2).max(8) });
